...
start_services(){ check_config;[ -d "$API_DIR/node_modules" ]||fail "API dependencies are missing; install explicitly";[ -d "$UI_DIR/node_modules" ]||fail "UI dependencies are missing; install explicitly";port_free "$BACKEND_PORT";port_free "$FRONTEND_PORT";(cd "$API_DIR" && PORT="$BACKEND_PORT" ./node_modules/.bin/ts-node --transpile-only src/index.ts) & api_pid=$!;(cd "$UI_DIR" && VITE_BACKEND_PORT="$BACKEND_PORT" VITE_FRONT_PORT="$FRONTEND_PORT" BROWSER=none npm run dev -- --host "${FRONTEND_HOST:-127.0.0.1}" --port "$FRONTEND_PORT") & ui_pid=$!;trap 'kill "$api_pid" "$ui_pid" 2>/dev/null || true;wait "$api_pid" "$ui_pid" 2>/dev/null || true' INT TERM EXIT;wait "$api_pid" "$ui_pid"; }
